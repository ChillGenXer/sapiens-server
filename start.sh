...
# Archive relevant logs generated by the world.
backup_logs(){
    # Create a backup filename with timestamp
    local timestamp=$(date +%Y-%m-%d-%H%M%S)
    local backup_file="log_backup_$timestamp.tar.gz"

    # Define full paths for tar operation
    local enet_log_dir=$(dirname "$ENET_LOG")
    local world_logs_parent_dir=$(dirname "$WORLD_LOGS_DIR")
